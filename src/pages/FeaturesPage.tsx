...
const STATUS_CONFIG: Record<FeatureStatus, { icon: React.ComponentType<{ className?: string }>; label: string; className: string; iconClassName: string }> = {
  completed: {
    icon: CheckCircle,
    label: 'Completed',
    className: 'bg-emerald-500/10 text-emerald-200 ring-1 ring-inset ring-emerald-400/30',
    iconClassName: 'text-emerald-600',
  },
  'in-progress': {
    icon: Clock,
    label: 'In progress',
    className: 'bg-amber-500/10 text-amber-200 ring-1 ring-inset ring-amber-400/30',
  ...
  experimental: {
    icon: Sparkles,
    label: 'Experimental',
    className: 'bg-purple-500/10 text-purple-200 ring-1 ring-inset ring-purple-400/30',
...
const categoryIconMap: Record<FeatureCategory, React.ComponentType<{ className?: string }>> = {
  core: Activity,
  gamification: Trophy,
  social: Users,
  ai: Brain,
...
const FeatureCard: React.FC<FeatureCardProps> = ({ feature, isExpanded, onToggleExpand }) => {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-2">
            <Typography variant="h3" className="text-lg font-semibold text-foreground group-hover:text-blue-600 transition-colors">
              {feature.title}
            </Typography>
        ...
        >
          {isExpanded ? (
            <span className="flex h-8 w-8 items-center justify-center text-lg text-foreground">-</span>
          ) : (
            <span className="flex h-8 w-8 items-center justify-center text-lg text-foreground">+</span>
          )}
        </Button>
        ...
        <div className="mt-6 space-y-5 rounded-2xl border border-border bg-muted/50 p-5 animate-slide-up">
          {feature.keyPoints && feature.keyPoints.length > 0 && (
            <div className="space-y-3">
              <Typography variant="h4" className="text-base font-semibold text-foreground">
                Key benefits
              </Typography>
              <ul className="space-y-2">
                {feature.keyPoints.map((benefit, index) => (
                  <li key={index} className="flex items-start gap-3 text-sm text-foreground/90 hover:text-foreground transition-colors group">
          ...
          {feature.longDescription && (
            <div className="space-y-2">
              ...
              <Typography variant="small" className="text-sm leading-relaxed text-muted-foreground">
                {feature.longDescription}
              </Typography>
            </div>
          )}
  ...
  const structuredData = {
    ...
    '@graph': [
      ...
      createWebPageSchema(
        ...
        typeof window !== 'undefined' ? window.location.href : 'https://liftfire.app/features',
        currentLanguage
      ),
    ],
  };
    ...
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section className="relative isolate overflow-hidden bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 dark:from-slate-900 dark:via-blue-950 dark:to-purple-950 pb-32 pt-36">
        <div className="pointer-events-none absolute inset-0">
          ...
          <div className="absolute -left-32 bottom-10 h-72 w-72 rounded-full bg-blue-500/20 blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
          <div className="absolute -right-36 top-24 h-80 w-80 rounded-full bg-purple-500/20 blur-3xl animate-pulse" style={{ animationDelay: '2s' }} />
        </div>
          ...
          <div className="grid gap-16 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                ...
                >
                  Discover everything that makes <span className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">LiftFire unbeatable</span>
                </Typography>
                <Typography
                  variant="lead"
              ...
              <div className="grid gap-4 sm:grid-cols-3 animate-slide-up" style={{ animationDelay: '0.3s' }}>
                {heroHighlights.map(highlight => {
                  const Icon = highlight.icon;
                  return (
                    <div key={highlight.title} className="rounded-2xl border border-border bg-card/50 backdrop-blur p-5 hover:bg-card hover:shadow-lg hover:border-blue-500/50 transition-all duration-300 group">
                      <Icon className="mb-3 h-6 w-6 text-blue-600 group-hover:scale-110 group-hover:rotate-3 transition-all" />
                      <p className="text-sm font-semibold text-foreground group-hover:text-blue-600 transition-colors">{highlight.title}</p>
              ...
              <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center animate-slide-up" style={{ animationDelay: '0.4s' }}>
                <Button variant="gradient" size="lg" asChild className="group px-8">
                ...
                <Button
                  ...
                  className="px-8"
                >
                  <Link to="/roadmap">{t('cta.secondary', { defaultValue: 'View roadmap' })}</Link>
                </Button>
              </div>
            </div>

            <div className="rounded-3xl border border-border bg-card/80 backdrop-blur-xl p-8 shadow-2xl animate-fade-in hover:shadow-blue-500/20 transition-shadow duration-500" style={{ animationDelay: '0.5s' }}>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <config.icon className={`h-5 w-5 ${config.iconClassName} group-hover:scale-110 transition-transform`} />
                        <span className="text-sm font-semibold text-foreground">
                          {config.label}
                        </span>
                      </div>
                      <span className="text-lg font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">{count}</span>
                    </div>
                  );
                })}
              </div>
              <div className="mt-6 rounded-2xl border border-border bg-gradient-to-r from-blue-500 to-purple-500 p-4 text-sm text-foreground hover:shadow-lg transition-shadow">
                We track every milestone publicly. Follow along in the roadmap as we roll new capabilities into beta each month.
              </div>
            </div>
          </div>
        </Container>
        ...
        <Container size="xl" padding="lg">
          <div className="rounded-3xl border border-border bg-card backdrop-blur-xl p-10 shadow-2xl">
            <div className="mb-10 space-y-6 text-center">
              <Typography variant="h2" className="text-3xl font-semibold text-foreground sm:text-4xl">
                {t('title')}
              </Typography>
              <Typography variant="lead" className="mx-auto max-w-3xl text-base text-muted-foreground sm:text-lg">
                {t('subtitle')}
              </Typography>
              <CategoryFilter
      ...
      <section className="relative pb-24">
        <Container size="xl" padding="lg" className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_0.9fr]">
          <Card
            variant="glass"
            padding="lg"
            className="border-border bg-muted/50 p-10 text-foreground backdrop-blur-xl"
          >
            <Typography variant="h2" className="text-3xl font-semibold text-foreground">
              {t('techStack.title')}
            </Typography>
            <Typography variant="body" className="mt-4 text-base text-muted-foreground">
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-muted/50 p-5">
                ...
                <p className="text-sm text-muted-foreground">{t('techStack.frontend')}</p>
              </div>
              <div className="rounded-2xl border border-border bg-muted/50 p-5">
                <div className="mb-4 flex items-center gap-3">
                ...
                <p className="text-sm text-muted-foreground">{t('techStack.backend')}</p>
              </div>
              <div className="rounded-2xl border border-border bg-muted/50 p-5">
                <div className="mb-4 flex items-center gap-3">
                ...
                <p className="text-sm text-muted-foreground">{t('techStack.security')}</p>
              </div>
              <div className="rounded-2xl border border-border bg-muted/50 p-5">
                <div className="mb-4 flex items-center gap-3">
                ...
                <p className="text-sm text-muted-foreground">{t('techStack.deployment')}</p>
              </div>
            </div>
          </Card>
          ...
          >
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-3">
              <div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/10 p-6 text-center">
                <p className="text-3xl font-bold text-foreground">{statusSummary.completed}</p>
                <p className="mt-2 text-sm font-medium text-emerald-600 dark:text-emerald-400">Completed features</p>
              </div>
              <div className="rounded-2xl border border-amber-400/20 bg-amber-500/10 p-6 text-center">
                <p className="text-3xl font-bold text-foreground">{statusSummary['in-progress']}</p>
                <p className="mt-2 text-sm font-medium text-amber-600 dark:text-amber-400">In development</p>
              </div>
              <div className="rounded-2xl border border-sky-400/20 bg-sky-500/10 p-6 text-center">
                <p className="text-3xl font-bold text-foreground">{statusSummary.planned}</p>
                <p className="mt-2 text-sm font-medium text-sky-600 dark:text-sky-400">Planned next</p>
              </div>
            </div>
            <Typography variant="body" className="mt-6 text-sm text-muted-foreground">
              Every release sharpens the training experienceï¿½from deeper analytics to smarter community tools. Follow our public roadmap to see exactly what ships next.
            </Typography>
          </Card>
        </Container>
      </section>
